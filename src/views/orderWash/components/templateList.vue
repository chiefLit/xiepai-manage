  ...
  <div class="template-list-wrapper">
    ...
    <el-table :data="dataList" style="width: 100%">
      ...
      <el-table-column label="下单时间">
        <template slot-scope="scope">{{scope.row.applyTime | parseTime}}</template>
      </el-table-column>
      <el-table-column prop="phone" label="用户名称" />
      <el-table-column prop="phone" label="手机号码" />
      <el-table-column prop="channel" label="渠道">
        <template slot-scope="scope">{{channelToValue(scope.row.channel)}}</template>
      </el-table-column>
      <el-table-column label="订单总价">
        <template
          slot-scope="scope"
        >{{scope.row.totalPrice ? scope.row.totalPrice.toFixed(2) : "--"}}</template>
      </el-table-column>
      <el-table-column label="用户实付金额">
        <template
          slot-scope="scope"
        >{{scope.row.realPayAmount ? scope.row.realPayAmount.toFixed(2) : "--"}}</template>
      </el-table-column>
